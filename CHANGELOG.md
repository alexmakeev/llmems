...
- **`ContextFactory`** (`src/services/context-factory.ts`) — the new entry point. Core API:
  ...
  - `getLongTermContext(sessionId)` — long-term-only projection: the same flat block as `getCurrentContext` but with the active raw tail EXCLUDED (only the stable backbone + dynamically-recalled mems). For consumers that already carry their own live session history (e.g. OpenCode) and would otherwise double-feed the latest turns.
